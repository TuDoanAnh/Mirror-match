...
// Function to copy pixels with HORIZONTAL FLIP (so left-facing sprite is converted to right-facing sprite)
function cropToCanvasFlipped(srcX, srcY, srcW, srcH, dstCanvas, dstX, dstY) {
  for (let cy = 0; cy < srcH; cy++) {
    for (let cx = 0; cx < srcW; cx++) {
      // Flip X coordinate within frame:
...
console.log(`Saved HORIZONTALLY FLIPPED ezreal_q_spritesheet.png (${qSheetW}x${qSheetH}, 5 frames)`);
...
spaceFrames.forEach((frame, i) => {
  ...
  cropToCanvasFlipped(frame.minX, frame.minY, w, h, spacePng, dstX, dstY);
});

fs.writeFileSync(path.join(outDir, 'ezreal_space_spritesheet.png'), PNG.sync.write(spacePng));
console.log(`Saved HORIZONTALLY FLIPPED ezreal_space_spritesheet.png (${spaceSheetW}x${spaceSheetH}, 7 frames)`);
